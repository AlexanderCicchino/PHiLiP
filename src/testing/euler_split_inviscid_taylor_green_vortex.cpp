...
template<int dim, int nstate>
double EulerTaylorGreen<dim, nstate>::compute_surface_cross_terms(const std::shared_ptr < DGBase<dim, double> > &dg, unsigned int poly_degree) const
{
   ...
   // const std::vector<double> &quad_weights_surf = dg->face_quadrature_collection[poly_degree].get_weights();
    ...
    //Face integral below
        ...
        for (unsigned int iface=0; iface < dealii::GeometryInfo<dim>::faces_per_cell; ++iface) {
            ...
            //get the surface reference 2pt flux
    ...
    std::array<std::vector<double>,nstate> surf_vol_ref_2pt_flux_interp_surf_int;
    std::array<std::vector<double>,nstate> surf_vol_ref_2pt_flux_interp_vol_int;
        std::array<dealii::FullMatrix<double>,nstate> surface_ref_2pt_flux_int;
        for(int istate=0; istate<nstate; istate++){
            surface_ref_2pt_flux_int[istate].reinit(n_face_quad_pts, n_quad_pts);
        }
        for(unsigned int iquad_face=0; iquad_face<n_face_quad_pts; iquad_face++){
            ...
            std::array<double,nstate> soln_state_face_int;
            soln_state_face_int = euler_double->compute_conservative_variables_from_entropy_variables (entropy_var_face_int);

            for (unsigned int iquad_vol=0; iquad_vol<n_quad_pts; ++iquad_vol) {
                ...
                dealii::Tensor<2,dim,double> metric_cofactor_vol_int;
                for(int idim=0; idim<dim; idim++){
                    for(int jdim=0; jdim<dim; jdim++){
                        metric_cofactor_vol_int[idim][jdim] = metric_oper.metric_cofactor_vol[idim][jdim][iquad_vol];
                    }
                }
                std::array<double,nstate> soln_state;
                for(int istate=0; istate<nstate; istate++){
                    soln_state[istate] = soln_at_q[istate][iquad_vol];
                }
                ...
                conv_phys_flux_2pt = euler_double->convective_numerical_split_flux(soln_state, soln_state_face_int);
                //Need to subtract off the pressure average term
                const double pressure_int = euler_double->compute_pressure(soln_state);
                const double pressure_ext = euler_double->compute_pressure(soln_state_face_int);
                for(int idim=0; idim<dim; idim++){
                    conv_phys_flux_2pt[1+idim][idim] -= 0.5*(pressure_int + pressure_ext);
                }

                for(int istate=0; istate<nstate; istate++){
                    dealii::Tensor<1,dim,double> conv_ref_flux_2pt;
        ...
        const std::vector<double> &oneD_quad_weights_vol_int = dg->oneD_quadrature_collection[poly_degree].get_weights();
        for(int istate=0; istate<nstate; istate++){
            surf_vol_ref_2pt_flux_interp_surf_int[istate].resize(n_face_quad_pts);
            surf_vol_ref_2pt_flux_interp_vol_int[istate].resize(n_quad_pts);
         
            flux_basis.surface_two_pt_flux_Hadamard_product(surface_ref_2pt_flux_int[istate], 
                                                                ...
                                                                iface,
                                                                dim_not_zero, 
                                                                unit_normal_int[dim_not_zero]);
        }


            //integrate the entropy contribution
        // Compute RHS
        for(int istate=0; istate<nstate; istate++){
            // interior RHS
            std::vector<double> rhs_int(n_shape_fns);
         
                std::vector<double> ones_surf(n_face_quad_pts, 1.0);
                soln_basis.inner_product_surface_1D(iface, 
                                                        ...
                                                        soln_basis.oneD_vol_operator,
                                                        false, -1.0);
                std::vector<double> ones_vol(n_quad_pts, 1.0);
                soln_basis.inner_product_1D(surf_vol_ref_2pt_flux_interp_vol_int[istate], 
                                                ones_vol, rhs_int, 
                                                soln_basis.oneD_vol_operator, 
                                                true, -1.0);
                for(unsigned int ishape=0; ishape<n_shape_fns; ishape++){
                    surf_cross_terms += energy_var_hat[istate][ishape] * rhs_int[ishape];
   ...
   // for (int i = 0; i < std::ceil(finalTime/dt); ++ i) {
    while(ode_solver->current_time < finalTime){
       // ode_solver->advance_solution_time(dt);
        ...
        std::cout << std::setprecision(16) << std::fixed;
//        pcout << "Energy at time " << ode_solver->current_time << " is " << current_energy / initial_energy << std::endl;
        pcout << "Energy mpi at time " << ode_solver->current_time << " is " << current_energy_mpi / initial_energy_mpi << std::endl;
//        myfile << ode_solver->current_time << " " << std::fixed << std::setprecision(16) << current_energy / initial_energy<< std::endl;
        myfile << ode_solver->current_time << " " << std::fixed << std::setprecision(16) << current_energy_mpi / initial_energy_mpi<< std::endl;
//        pcout << "Actual Energy Divided by volume at time " << ode_solver->current_time << " is " << current_energy/(8*pow(dealii::numbers::PI,3)) << std::endl;
 ...
 //       double current_entropy_mpi = sqrt(abs(dealii::Utilities::MPI::sum(current_entropy, mpi_communicator)));
        std::cout << std::setprecision(16) << std::fixed;
        pcout << "M plus K norm Entropy at time " << ode_solver->current_time << " is " << current_entropy_mpi / initial_entropy_mpi<< std::endl;
        ...
        double current_change_entropy_mpi = (dealii::Utilities::MPI::sum(current_change_entropy[0], mpi_communicator));
        double current_change_energy_mpi = (dealii::Utilities::MPI::sum(current_change_entropy[1], mpi_communicator));
        std::cout << std::setprecision(16) << std::fixed;
...
//        myfile << ode_solver->current_time << " " << std::fixed << std::setprecision(16) << current_change_entropy[0]<< std::endl;
//        myfile << ode_solver->current_time << " " << std::fixed << std::setprecision(16) << current_change_entropy[1]<< std::endl;
        myfile << ode_solver->current_time << " " << std::fixed << std::setprecision(16) << current_change_entropy_mpi<< std::endl;
        myfile << ode_solver->current_time << " " << std::fixed << std::setprecision(16) << current_change_energy_mpi<< std::endl;
        ...
        myfile << ode_solver->current_time << " " << std::fixed << std::setprecision(16) << current_change_entropy[1]-current_p_work_mpi<< std::endl;

//        double current_cross_work = compute_surface_cross_terms(dg, poly_degree);
//        double current_cross_work_mpi = (dealii::Utilities::MPI::sum(current_cross_work, mpi_communicator));
//        pcout<<"Current change energy pressure work cross terms "<<current_change_entropy[1]-current_p_work_mpi-current_cross_work_mpi<<std::endl;
