#ifndef __HIT_CHAI_MAHESH_H__
#define __HIT_CHAI_MAHESH_H__
...
class HITChaiMahesh : public TestsBase
{
public:
    /// Constructor.
    /** Simply calls the TestsBase constructor to set its parameters = parameters_input
     * */
    HITChaiMahesh(const Parameters::AllParameters *const parameters_input);
...
//    double compute_entropy(std::shared_ptr < DGBase<dim, double> > &dg, unsigned int poly_degree) const;
    ...
    /// Sets the IC from txt files.
    double set_initial_condition_from_file(std::shared_ptr < DGBase<dim, double> > &dg, unsigned int poly_degree, const double delta_x) const;
};
