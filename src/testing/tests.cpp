...
::select_test(const AllParam *const parameters_input,
    ...
    } else if(test_type == Test_enum::inviscid_vortex_shock_interaction) {
        if constexpr (dim==2 && nstate == dim+2) return std::make_unique<InviscidVortexShockInteraction<dim,nstate>>(parameters_input);
    } else {
